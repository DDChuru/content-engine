...
const output=path.join(root,'out/remedial-action-candidate-v3');fs.mkdirSync(output,{recursive:true});
...
async function main(){
 ...
 for(const [mutate,reason] of [
  ...
  ...['two-outcomes','finding-origin','inspection-capture','critical-handoff'].map(id=>[x=>x.beats.push(require(path.join(source,'narration.json')).beats.find(b=>b.id===id)),/V3 excludes Daily backlog/]),
  [x=>x.beats.find(b=>b.id==='open-card').narration='Create a new finding.',/existing finding bridge/],
  [x=>x.beats.find(b=>b.id==='resolved').cues[0].proof='summary',/no Daily backlog or creation capture/],
  [x=>x.beats.find(b=>b.id==='inspection-review').narration+=' Save finding before Sign and Complete.',/no Daily backlog or finding-creation/],
 ...
 browser.newPage=async(...args)=>{
  ...
  client.send=async(method,...args)=>{
   if(method==='Page.captureScreenshot'&&activeSample){
    ...
    if(['resolved','audit-trail'].includes(beat.id)){const phones=await page.evaluate(()=>document.querySelectorAll('[data-remedial-app]').length);assert.equal(phones,0,'closed record must not show the surrounding Daily backlog');}
 ...
 const file=path.join(output,'remedial-action-review-candidate-v3.mp4');
 ...
 const aligned=path.join(output,'remedial-action-review-candidate-v3-audio-aligned.mp4');
