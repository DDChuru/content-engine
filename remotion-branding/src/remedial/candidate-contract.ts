...
export const CANDIDATE_ID = 'RemedialActionTutorial-REVIEW-CANDIDATE';
...
export const CANDIDATE_SHA256 = '3007d4678c4d9f5c3337f1167c845a78cca1bd9faf22ae43a10edb9f213d6f34';
...
export const assertCandidateReady = (candidate: Candidate, narration: NarrationManifest) => {
	...
	check(narration.beats.length === 24 && same(narration.beats.map((b) => [b.id, b.kind]), planned.beats.map((b) => [b.id, b.kind])), 'all original storyboard beats required');
	...
	check(carry.narration.includes('not a later-day comparison') && carry.note.includes('No aged Inspection record'), 'later-day limitation must be audible and visible');
	check(candidate.narration.clips.length === 22 && candidate.music.sha256 === 'f1928a7b68b79b89c843af517583ddc636773e8c4a354e3b610d42611962d186', 'established narration/bookend music required');
};
