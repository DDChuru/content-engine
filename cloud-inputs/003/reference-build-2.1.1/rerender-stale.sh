...
STALE=$(node -e "const fp=require('./beat-fingerprint.cjs'),fs=require('fs');const o=[];for(let i=1;i<=20;i++){const f='./render-cache/beat-'+String(i).padStart(2,'0')+'/complete.json';if(fs.existsSync(f)&&require(f).sourceHash!==fp(i))o.push(i);}console.log(o.join(' '))")
echo "stale: $STALE"
for i in $STALE; do
  N=$(printf '%02d' $i)
