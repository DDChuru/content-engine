...
Cue phrases are exact narration substrings, unique in their beat and in order. A '^' inside a
phrase marks the trigger word (the phrase is still matched whole, for uniqueness)."""
...
for b in script:
    ...
    if not mp.exists() or not wp.exists(): print('Awaiting measured/transcribed beat',b['id']);sys.exit(2)
    ...
    for n,(phrase,key,cap) in enumerate(PLANS[b['id']]):
        raw=norm(phrase.replace('^',' ^'));trig=0
        k=0
        for t in raw:
        ...
        w=obsw[mapping[candidate]];ct=max(0,w['start'])
        if cap: caption=cap
        cue={'id':f"B{b['id']:02d}C{n+1:02d}",'beat':b['id'],'phrase':phrase.replace('^',''),'key':key,'trigger':w['word'],'wordStart':w['start'],'localTime':round(ct,6),'time':round(sample_cursor/48000+ct,6),'caption':caption,'match':'exact-spoken-token','sourceIndex':candidate,'triggerOffset':candidate-ix}
...
gaps=[]
for sc in scenes:
    points=[0]+[c['localTime'] for c in sc['cues']]+[sc['audioDuration']]
...
print(json.dumps({k:T[k] for k in ['duration','narrationSeconds','words','wpm','durationFrames']}));print('cues',len(allc),'maxgap',D['maxCueGap'],'over12',D['gapsOver12'])
