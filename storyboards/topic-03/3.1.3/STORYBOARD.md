...
- **`sealed`**: after the 10 min equilibration, with the tap set to **vent**, the bung is **seated firmly before mixing**, delivery tube and flexible tubing connected through the tap to the syringe, the syringe having been set to 0 cm³ with its nozzle open to the air before connection (a preparation, not a reaction reading), connected at 0 cm³ while the tap is at vent; the isolated syringe's plunger is checked to be still at 0 cm³, without moving it against the closed syringe port, and the tap turned to **collect**; tag *baseline 0 cm³ verified after equilibration; collection path gas-tight*, then *system closed*. The bung stays seated and the tap stays at collect through every later state.
- **`tilt`** (one atomic start event): a hand holds the flask **by its neck, above the weighted ring**, lifts it just clear of the rack (its body still in the bath water) and tilts it smoothly toward the low side to about **30° from upright**; the peroxide surface stays horizontal and pools at the low heel. **Toppling path of the small tube:** as the flask passes about 20°, the small tube pivots on its downhill base edge, swings downhill and comes to lie along the flask base with its mouth toward the low heel (about 4 mm clear of the low wall), its axis at about **120° from upright (within 110–130°), mouth below base**. During toppling, the liquid surface stays horizontal and suspension begins leaving the rotated lip as soon as the liquid reaches it; do not delay spilling until the tube passes horizontal. Show the suspension entering and mixing with the peroxide. Where the tube mouth becomes submerged in the pool, show liquid continuity through the opening and reaction mixture inside the immersed part of the tube; do not draw that part empty. **On the rendered frame on which the first yeast suspension touches the peroxide, the stopwatch starts and collection begins; that frame is t = 0.** Any later time-zero tag or cue highlights the already-running stopwatch; it is never reset. The flask is then returned upright to the bath over ~0.5 s, the small tube lying on the flask base, with reaction mixture visible inside wherever it is connected to the surrounding pool.
...
3. At *plunger at zero*, the syringe scale zooms briefly: the plunger, prepared at 0 cm³ with its nozzle open to the air before connection, is checked to be still at **0 cm³** without moving it, and the tap turned to **collect**, tag *baseline 0 cm³ verified after equilibration; collection path gas-tight*; at *The system is now closed*, a thin outline traces flask → bung → tubing → tap → syringe, tag **system closed before mixing**.
4. At *Tilt the flask by its neck*, `tilt`: a hand takes the neck above the weighted ring, lifts the flask just clear of the rack (body still in the bath water) and tilts it to about 30° from upright; the bung stays seated; the peroxide surface stays horizontal and pools at the low heel. At *the yeast spills into the peroxide*, the small tube follows its specified toppling path to lie along the flask base at about 120° from upright, mouth below base; suspension leaves its rotated lip during the topple and mixes into the peroxide pool; when the mouth becomes immersed, liquid continuity and reaction mixture inside the immersed part of the tube remain visible. **On the rendered frame on which the first yeast suspension touches the peroxide, the stopwatch starts and collection begins** (one atomic event; it is not held for a later cue).
...
| Item | Change |
|---|---|
| N1.1 (`GasSyringeRig` `tilt`) | The two sentences "As it passes horizontal … residual film." replaced verbatim with the checker's text ("During toppling, the liquid surface stays horizontal … do not draw that part empty."). |
| N1.2 (same state, last sentence) | "the emptied small tube lying on its base" → "the small tube lying on the flask base, with reaction mixture visible inside wherever it is connected to the surrounding pool". |
| N1.3 (Beat 6 action 4) | "a stream leaves its rotated lip and it visibly empties into the peroxide pool" → "suspension leaves its rotated lip during the topple and mixes into the peroxide pool; when the mouth becomes immersed, liquid continuity and reaction mixture inside the immersed part of the tube remain visible". The first-contact start instruction that follows is unchanged. |
| Length | Accepted by the checker (11:49); no narration change. |

Round two: CLEARED WITH MINOR EDITS; with N1 applied, treated as CLEARED.

## CHECK RESPONSE (round 2, carried from the 3.2.1 check)

The 3.2.1 round-two check (correction A) found that setting an isolated syringe's plunger to zero with the tap at vent is ambiguous, and asked for the same preparation/verification wording in the shared rig; it named this file's `sealed` state and Beat 6. Applied: the syringe is set to 0 cm³ with its nozzle open to the air **before connection**; after equilibration its plunger is **checked, not moved**, then the tap is turned to collect (model `sealed`, Beat 6 action 3, Dataset 1 conditions); tag "baseline 0 cm³ set after equilibration" → "baseline 0 cm³ verified after equilibration" in current text. No narration, cue or data change.
