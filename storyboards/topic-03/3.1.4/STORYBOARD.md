...
**Handling contract (specified; rendered still-frame verification pending).** The cuvette is held between finger and thumb **on the ridged faces only**; fingers never touch the clear faces. Liquids are dispensed from a graduated pipette whose tip is inside the cuvette mouth, above the liquid surface, not touching the clear faces. Mixing: a cuvette lid is fitted and the cuvette is inverted twice, held by the ridged faces, liquid surface level after it is set upright. The cuvette is lowered into the holder clear faces to the light path, and the holder lid is closed before any reading. Between readings the cuvette stays in the holder (fixed light path).
...
Round two: **CLEARED**. One non-blocking housekeeping item applied: the heading "Handling contract (checked on a still frame)" → "Handling contract (specified; rendered still-frame verification pending)". No narration, data or cue change.
