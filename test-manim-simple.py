...
"""
Simple Manim test - Circle Theorem without LaTeX issues
"""

from manim import *

class SimpleCircleTest(Scene):
    def construct(self):
        # Title
        title = Text("Angle at Centre Theorem", font_size=40)
        title.to_edge(UP)
        ...
        self.play(
            ...
            FadeIn(dot_C, label_C)
        )
        self.wait(0.5)

        # Draw angle at centre (O)
        line_OA = Line(center.get_center(), point_A, color=GREEN)
        line_OB = Line(center.get_center(), point_B, color=GREEN)
        ...
        # Explanation box
        explanation = VGroup(
            Text("Centre = 2 × Circumference", font_size=32),
            Text("120 = 2 × 60", font_size=28, color=YELLOW)
