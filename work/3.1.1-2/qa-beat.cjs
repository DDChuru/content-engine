...
const P=__dirname;process.env.FONTCONFIG_FILE=P+'/fonts/fonts.conf';process.env.XDG_CACHE_HOME=P+'/render-cache';process.env.RAYON_NUM_THREADS='2';
const fs=require('fs'),React=require('react'),{renderToStaticMarkup}=require('react-dom/server'),sharp=require('sharp'),normalise=require('./raster-svg.cjs'),{Lesson,stateAt,isError}=require('./render-cache/Lesson.cjs'),T=require('./timeline.json'),fingerprint=require('./beat-fingerprint.cjs');sharp.concurrency(2);
...
(async()=>{const entries=sc.cues.map((c,i)=>({id:c.id+'-'+c.key,frame:sc.startFrame+Math.round(Math.min(c.localTime+.8,sc.cues[i+1]?sc.cues[i+1].localTime-.04:sc.duration-.04)*30)}));entries.push({id:'START',frame:sc.startFrame},{id:'END',frame:sc.startFrame+sc.frames-1});
for(const x of extra)entries.push({id:'T'+x.toFixed(1),frame:sc.startFrame+Math.round(x*30)});
for(let k=1;k<sc.frames;k++){const f=sc.startFrame+k;if(isError(stateAt(f))!==isError(stateAt(f-1)))entries.push({id:'BEFORE-CLEAR',frame:f-1},{id:'CLEAR',frame:f});}
