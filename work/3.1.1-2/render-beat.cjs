...
const P=__dirname;
process.env.RAYON_NUM_THREADS='1';process.env.UV_THREADPOOL_SIZE='2';process.env.TMPDIR=P+'/tmp';process.env.FONTCONFIG_FILE=P+'/fonts/fonts.conf';process.env.XDG_CACHE_HOME=P+'/render-cache';
...
sharp.concurrency(1);sharp.cache({memory:96,files:0,items:64});fs.mkdirSync(P+'/tmp',{recursive:true});
const {Lesson,stateAt,isError,ERROR_LABEL,audits}=require('./render-cache/Lesson.cjs'),T=require('./timeline.json');const id=Number(process.argv[2]),sc=T.scenes.find(s=>s.id===id);if(!sc)throw Error('Unknown beat');
...
(async()=>{try{fs.writeFileSync(dir+'/render.lock',String(process.pid),{flag:'wx'});}catch(e){if(e.code!=='EEXIST')throw e;throw Error('render.lock exists for '+name+': check the PID inside; never delete a live lock');}
 ...
 let prev='',unique=0,records=[],last=-1,start=Date.now();const count=sc.frames+(id===T.scenes.length?30:0);const audit=[];
 for(let k=0;k<count;k++){
  ...
  if(audits[id]){const r=audits[id](stateAt(frame));if(r){if(!r.ok)throw Error('Valence audit failed at frame '+frame+': '+JSON.stringify(r));audit.push({frame,...r});}}
 ...
 if(audit.length)fs.writeFileSync(dir+'/valence-audit.json',JSON.stringify({framesAudited:audit.length,allOk:audit.every(a=>a.ok),states:[...new Set(audit.map(a=>a.state))],frames:audit}));
