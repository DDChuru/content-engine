"""Join the 18 beat chunks (-c copy) with the once-encoded narration AAC (-c copy)."""
from pathlib import Path
import subprocess,json,sys
P=Path(__file__).resolve().parent
T=json.loads((P/'timeline.json').read_text())
assert len(T['scenes'])==18
...
for sc in T['scenes']:
    ...
    want=sc['frames']+(30 if sc['id']==18 else 0)
...
out=P/'3.1.1-2-enzymes.mp4';tmp=P/'3.1.1-2-enzymes.partial.mp4'
