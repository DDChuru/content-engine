...
The credential is injected by the environment's proxy: no key header is sent or stored."""
from pathlib import Path
import json,subprocess,urllib.request,urllib.error,wave,hashlib,sys,re
...
# Km: probe (audio/probe) — the bare token was voiced as a word ("Kamihamj"/"Kynum" in transcript); the letters are wanted.
REQUEST_NORMALISE=[(r'\bKm\b','K M')]
# Per-beat: B9's first take voiced "Km with Vmax" as "Weimags" (small and medium.en agree); the letters are wanted.
REQUEST_NORMALISE_BEAT={9:[(r'\bVmax\b','V max')]}
def request_text(t,beat=None):
    for a,b in REQUEST_NORMALISE+REQUEST_NORMALISE_BEAT.get(beat,[]): t=re.sub(a,b,t)
    return t
...
for b in json.loads((P/'script.json').read_text()):
    ...
    body=dict(text=request_text(b['text'],b['id']),model_id='eleven_multilingual_v2',voice_settings=SETTINGS)
