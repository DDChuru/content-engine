...
- At most three regions including the header: one graph and one card or lined-paper panel. Never pair two separate graphs. At most twelve words per card or handwritten line. One new annotation or card per spoken cue; erase previous annotations between legs.
...
- All visual changes follow locally transcribed words. Editorial durations below are budgets; final timings use encoded narration. Slow S04 and S07 use ElevenLabs speed 0.9; every other scene uses 1.0. No other slow scenes.
...
- Use the source cyclist data: constant +6 m/s for 10 s; uniform deceleration to rest over 12 s; rest 5 s; uniform reverse acceleration to −4 m/s over 4 s; constant return to O. Key times 0,10,22,27,31,53; displacement 0,60,96,96,88,0.
...
**Narration:** “On velocity time, gradient gives acceleration: a straight slope means a constant rate of change. Area gives displacement, because velocity accumulates over time. Below the axis, area is negative. On displacement time, gradient gives velocity. A falling line means returning, not a negative distance travelled.”

**Viewer sees:** One abstract graph at a time, without numerical examples. Velocity-time: reveal a straight rising line, then its triangular area; replace with a below-axis region at negative. On displacement-time, replace the graph with a straight falling segment and one short card. Cards: `Gradient gives acceleration`, `Signed area gives displacement`, `Below-axis area is negative`, `Gradient gives velocity`, `Negative velocity means returning`.

## S03 — Draw the cyclist’s velocity
...
**Narration:** “A cyclist passes O, rides out, stops, then returns. We'll draw velocity against time, taking outward as positive; each phrase chooses a shape. Let's get drawing. Six metres per second for ten seconds: horizontal. Slowing uniformly over twelve seconds reaches rest at twenty-two: a straight fall. Resting five seconds takes us to twenty-seven. Accelerating backwards for four seconds reaches minus four at thirty-one. Keep minus four until returning to O at T.”

**Viewer sees:** Bare velocity-time axes through the complete setup. Then draw five segments individually at their spoken phases. Reveal each endpoint time/value when said. Each short card replaces its predecessor: `Constant velocity`, `Uniform deceleration`, `At rest`, `Accelerating in reverse`, `Constant return velocity`. The final time label is T until the area calculation finds it. No working during setup.
...
**Narration:** “For the same cyclist, find the return time. Returning to O means the outward and return areas match. Let's get drawing. [pause 2 s] The rectangle gives sixty metres. [hold 2 s] The triangle adds thirty-six metres. [hold 2 s] Together, ninety-six metres outward. [hold 2 s] The reverse triangle accounts for eight metres. [hold 2 s] That leaves eighty-eight metres to return. [hold 2 s] At four metres per second, that's twenty-two more seconds. [hold 2 s] Add the first thirty-one: T is fifty-three seconds. Back at O. [hold 2 s]”
...
## S05 — Choose straight lines or curves
...
**Narration:** “Constant velocity gives a straight displacement line: equal changes in equal times. Rest is horizontal. Changing velocity gives a curve. A straight join would hide its changing gradient.”

**Viewer sees:** Reuse qualitative portions of the cyclist displacement graph, one shape at a time. One card: `Constant velocity: straight`, `Rest: horizontal`, `Changing velocity: curved`. At the straight-line mistake, show a dashed chord over the existing reverse curve; no new system or calculation.
...
**Narration:** “If velocity is negative, must displacement be negative? [hold 3 s] No. Our returning cyclist stays beyond O until arriving back. You can turn journey descriptions into graph shapes; use signed area to find displacement; and explain a journey using displacement time gradients. Now apply all three.”
