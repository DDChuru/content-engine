/* Still-only M4.2 audit: visual pixels, layout, pen completion, holds and spoken rings. */
const fs=require('node:fs'),path=require('node:path'),assert=require('node:assert/strict'),crypto=require('node:crypto');
const {bundle}=require('@remotion/bundler');
const {openBrowser,selectComposition,renderStill}=require('@remotion/renderer');
const sharp=require('sharp');
const root=path.resolve(__dirname,'../..');
const [prefix='using-calculus-in-1d',id='MechanicsUsingCalculusIn1D',entry='verify-calculus-entry.tsx']=process.argv.slice(2);
...
(async()=>{
 ...
 assert.equal(composition.width,1920);assert.equal(composition.height,1080);assert.equal(composition.fps,30);assert(composition.durationInFrames>=8100&&composition.durationInFrames<=10800,'Duration must be 4:30–6:00');
