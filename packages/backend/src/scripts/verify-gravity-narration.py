  ...
  for s in scenes:
   ...
   for b in s['beats']:
    ...
    if cache.exists():local=json.loads(cache.read_text())
     ...
     segs,_=model.transcribe(np.frombuffer(pcm,dtype=np.float32),language='en',word_timestamps=True,beam_size=1,best_of=1,vad_filter=False,initial_prompt=b['text'],condition_on_previous_text=False)
     local=[dict(word=w.word.strip(),start=round(b['start']+w.start,3),end=round(min(b['speechEnd'],b['start']+w.end),3)) for seg in segs for w in (seg.words or [])]
     cache.write_text(json.dumps(local,indent=2)+'\n')
    # These two short clips lose words or add boilerplate in isolation. Their
    # complete-scene local pass preserves the spoken phrase without that error.
    if (s['id'],b['id']) in {('s04','lower'),('s08','outcome3')}:
     reference=json.loads(full_cache.read_text())['words']
     local=[{**w,'start':max(b['start'],w['start']),'end':min(b['speechEnd'],w['end'])} for w in reference if b['start']<=(w['start']+w['end'])/2<b['speechEnd']]
