#!/usr/bin/env python3
"""Generate Displacement Time Graphs narration with exact inserted-silence holds."""
...
def scenes():
    source=(ROOT/'packages/backend/projects/mechanics-displacement-time-graphs/STORYBOARD.md').read_text()
    jobs=[]
    for section in re.split(r'(?=^## S\d\d)',source,flags=re.M)[1:]:
        section=section.split('## Additive visual contract')[0]
        sid=re.search(r'## (S\d\d)',section)[1].lower()
        if sid not in ('s00','s07','s08','s09','s11'): continue
        paragraphs=re.findall(r'^\*\*Narration:\*\* “([^”]+)”',section,re.M)
        jobs.append(dict(id=sid,paragraphs=paragraphs,budget=80 if sid=='s11' else 0,voiceSpeed=1.0 if sid=='s00' else .9,tempo='brisk' if sid=='s00' else 'slow'))
    return jobs
...
def generate(scene):
    ...
    for paragraph in scene['paragraphs']:
        ...
        silence(0.05 if scene['id']=='s11' else 0.15,'breath')
    silence(max(0.3,scene['budget']-position/SAMPLE_RATE),'tail')
    target=AUDIO/f"displacement-time-graphs-{scene['id']}.mp3"
...
if __name__=='__main__':
    ...
    if len(sys.argv)>1:
        ...
        print('GENERATED',sum(s['sampleDuration'] for s in result))
