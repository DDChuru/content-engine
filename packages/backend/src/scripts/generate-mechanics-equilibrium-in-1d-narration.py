#!/usr/bin/env python3
"""Generate Equilibrium in 1D narration with exact inserted-silence holds."""
import concurrent.futures, hashlib, json, os, re, subprocess, sys
from pathlib import Path
import numpy as np
from dotenv import load_dotenv
load_dotenv(Path.home()/"Documents/projects/content-engine/.env")
ROOT=Path(__file__).resolve().parents[4]
WORK=Path('/tmp/verify-equilibrium-narration'); WORK.mkdir(exist_ok=True)
AUDIO=ROOT/'packages/backend/src/remotion/public/audio/mechanics'
SAMPLE_RATE=44100

def scenes():
    source=(ROOT/'packages/backend/projects/mechanics-equilibrium-in-1d/STORYBOARD.md').read_text()
    result=[]
...
def generate(scene):
    ...
    target=AUDIO/f"equilibrium-in-1d-{scene['id']}.mp3"
