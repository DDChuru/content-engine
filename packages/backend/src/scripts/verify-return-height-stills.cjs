/* S04 return-height revision: targeted stills, holds/rings, plus unchanged-scene end checks. */
...
const output=path.join(root,'out','verify-return-height-stills');
...
(async()=>{
 ...
 const serveUrl=await bundle({entryPoint:path.join(__dirname,entry),publicDir:null,webpackOverride:config=>({...config,resolve:{...config.resolve,modules:[process.env.NODE_PATH,'node_modules'].filter(Boolean)}}),outDir:path.join(root,'out','verify-return-height-bundle')});
 ...
 assert.equal(composition.width,1920);assert.equal(composition.height,1080);assert.equal(composition.fps,30);assert.equal(composition.durationInFrames,transcript.scenes.reduce((n,s)=>n+Math.ceil(s.duration*30),0));
 ...
 for(const s of transcript.scenes){offsets[s.id]=offset;if(s.id!=='s04'){add(offset+Math.ceil(s.duration*30)-1,s.id+':unchanged-end');offset+=Math.ceil(s.duration*30);continue;}add(offset,s.id+':start');add(offset+Math.ceil(s.duration*30)-1,s.id+':end');
