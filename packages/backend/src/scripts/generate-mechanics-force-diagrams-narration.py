#!/usr/bin/env python3
"""Generate Force Diagrams narration with exact inserted-silence holds."""
...
WORK=Path('/tmp/verify-force-diagrams-narration'); WORK.mkdir(exist_ok=True)
AUDIO=ROOT/'packages/backend/src/remotion/public/audio/mechanics'
SAMPLE_RATE=44100

def scenes():
    source=(ROOT/'packages/backend/projects/mechanics-force-diagrams/STORYBOARD.md').read_text()
    result=[]
...
def generate(scene):
    ...
    target=AUDIO/f"force-diagrams-{scene['id']}.mp3"
