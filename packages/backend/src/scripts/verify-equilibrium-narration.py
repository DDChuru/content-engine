...
T=ROOT/'packages/backend/src/remotion/public/transcripts/mechanics/equilibrium-in-1d.json'
...
def main():
    ...
    for s in data['scenes']:
        ...
        total+=s['duration']
        rows.append({'scene':s['id'],'duration':s['duration'],'words':len(s['words']),'cues':len(s['cues']),'figures':len(s['figures']),'holds':holds})
    assert 210<=total<=270
