#!/usr/bin/env python3
"""SUVAT in 1D narration with inserted holds and isolated local Whisper word cues."""
...
def main():
 ...
 if not args.transcribe:
  for line in Path(os.environ.get('CONTENT_ENGINE_ENV',Path.home()/'Documents/projects/content-engine/.env')).read_text().splitlines():
  ...
  for s in scenes:
   ...
   for b in s['beats']:
    ...
    if cache.exists() and json.loads(cache.read_text())['sha']==s['audioSha256']:
     local=json.loads(cache.read_text())['words']
    else:
     clip=work/(s['id']+'-'+b['id']+'-verify.wav')
     subprocess.run(['ffmpeg','-v','error','-y','-ss',str(b['start']),'-i',str(A/s['audio']),'-t',str(b['speechEnd']-b['start']),'-ar','16000','-ac','1',str(clip)],check=True)
     segments,_=model.transcribe(str(clip),language='en',word_timestamps=True,beam_size=5,vad_filter=False,initial_prompt=b['text'],condition_on_previous_text=False)
     local=[dict(word=w.word.strip(),start=round(b['start']+w.start,3),end=round(min(b['speechEnd'],b['start']+w.end),3)) for segment in segments for w in (segment.words or [])]
     cache.write_text(json.dumps(dict(sha=s['audioSha256'],words=local),indent=2)+'\n')
    # This quiet second sentence needs its own local decoder context.
    if s['id']=='s07' and b['id']=='second-list' and local[-1]['end']<b['speechEnd']-1:
     clip=work/(s['id']+'-'+b['id']+'-verify.wav')
     segments,_=model.transcribe(str(clip),language='en',word_timestamps=True,beam_size=5,vad_filter=False,clip_timestamps=[4.0,b['speechEnd']-b['start']],initial_prompt='Keep the same u and a.',condition_on_previous_text=False)
     tail=[dict(word=w.word.strip(),start=round(b['start']+w.start,3),end=round(min(b['speechEnd'],b['start']+w.end),3)) for segment in segments for w in (segment.words or [])]
     assert tail,'Missing locally transcribed shared-variable sentence'
     local=[w for w in local if w['start']<b['start']+4.0]+tail
     cache.write_text(json.dumps(dict(sha=s['audioSha256'],words=local),indent=2)+'\n')
