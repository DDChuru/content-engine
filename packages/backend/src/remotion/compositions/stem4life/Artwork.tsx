...
const Microscope: React.FC<{frame: number; fps: number; accent: string; draw?: boolean}> = ({frame, fps, accent, draw}) => {
  const id = useId().replace(/:/g, '');
  return <g>
    {MICROSCOPE_PATHS.map((d, i) => {
      const p = draw ? progress(frame, fps, [0.25, 0.05, 0.72][i], [0.86, 0.58, 0.42][i]) : 1;
      return <g key={i}>
