/**
 * Generate per-beat narration for the e-wizer Bill of Health tutorial (Ecowize Academy video 07).
 * Daniel voice (src/boh/narration.json voiceSettings) — one MP3 per beat so the Remotion
 * composition can compute drift-free voStart times from real durations.
 *
 * Run: npx tsx packages/backend/scripts/generate-boh-tutorial-audio.ts
 */
...
const CONFIG_PATH = path.join(RB, 'src/boh/narration.json');
const AUDIO_DIR = path.join(RB, 'public/boh/audio');
const TIMING_PATH = path.join(RB, 'src/boh/timing.json');
