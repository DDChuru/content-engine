/**
 * Generate per-beat narration for the e-wizer CCV refresh (Ecowize Academy video 03).
 * Daniel voice (src/ccv2/narration.json voiceSettings) — one MP3 per beat so the Remotion
 * composition can compute drift-free voStart times from real durations.
 *
 * Run: npx tsx packages/backend/scripts/generate-ccv2-tutorial-audio.ts
 */
...
const CONFIG_PATH = path.join(RB, 'src/ccv2/narration.json');
const AUDIO_DIR = path.join(RB, 'public/ccv2/audio');
const TIMING_PATH = path.join(RB, 'src/ccv2/timing.json');
